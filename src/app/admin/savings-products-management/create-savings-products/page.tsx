...
import { adminCreateSavingsProduct, adminGetAllLoaiSoTietKiemDanhMuc } from "@/services/api";
import ProtectedRoute, { Role } from "@/components/ProtectedRoute";

export default function CreateSavingsProductPage() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                setLoading(true);
                setError(null);
                const data = await adminGetAllLoaiSoTietKiemDanhMuc();
                setCategories(Array.isArray(data) ? data : []);
            } catch (err: unknown) {
                const errorMessage = err instanceof Error ? err.message : "Không thể tải danh mục sản phẩm.";
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            await adminCreateSavingsProduct(formData);
            setSuccessMessage("Tạo sản phẩm tiết kiệm thành công!");
            setTimeout(() => {
                router.push("/admin/savings-products-management");
            }, 1500);
        } catch (err: unknown) {
            const errorMessage = err instanceof Error ? err.message : "Tạo sản phẩm thất bại. Vui lòng thử lại.";
            setError(errorMessage);
        } finally {
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-200 py-3">
                                ...
                                <input
                                    ...
                                    className="w-full sm:w-3/5 md:w-2/3 text-base text-gray-800 bg-white border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 mt-1 sm:mt-0"
                                />
                            </div>                            {/* Tiền gửi ban đầu tối thiểu */}
                            <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-200 py-3">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-200 py-3">
                                ...
                                <input
                                    ...
                                    className="w-full sm:w-3/5 md:w-2/3 text-base text-gray-800 bg-white border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 mt-1 sm:mt-0"
                                />
                            </div>                            {/* Số ngày gửi tối thiểu để rút */}
                            <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-200 py-3">
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-200 py-3">
                                ...
                                <select
                                    ...
                                    className="w-full sm:w-3/5 md:w-2/3 text-base text-gray-800 bg-white border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-pink-500 focus:border-pink-500 mt-1 sm:mt-0"
                                >
                                    <option value="">Chọn loại danh mục</option>                                    {categories.map((category) => (
