...
export default function UpdateAdminProfilePage() {
    ...
    const [currentAdminId, setCurrentAdminId] = useState<number | null>(null);    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                setCurrentAdminId(data.id); // Lưu id admin hiện tại
            } catch (err: any) {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (currentAdminId) {
                await adminUpdateUserByAdmin(currentAdminId, dataToUpdate);
                ...
                setError("Không xác định được tài khoản admin hiện tại.");
            }
        } catch (err: any) {
        ...
        <div className="min-h-screen flex flex-col bg-gradient-to-r from-[#FF086A] via-[#FB5D5D] to-[#F19BDB]">
            ...
            <div className="flex-1 flex items-center justify-center pt-20 md:pt-24 px-4 pb-10">
                <div className="w-full max-w-xl lg:max-w-2xl bg-white rounded-2xl shadow-2xl p-6 sm:p-8 md:p-10 my-8">
                    ...
                    {successMessage && <p className="text-green-600 bg-green-100 p-3 rounded-md mb-4 text-sm text-center">{successMessage}</p>}                    <form onSubmit={handleSubmit} className="space-y-4">
                        {[
